...
export default function BlogPageClient() {
  ...
  const filteredPosts = useMemo(() => {
    return blogPosts.filter((post) => {
      ...
      if (activeCountryFilter === "all") countryMatch = true;
      else if (activeCountryFilter === "general") countryMatch = post.country === "All" || post.country === "General";
      else countryMatch = post.country === activeCountryFilter.toUpperCase();
  ...
  useEffect(() => { setVisibleCount(6); setLoadingMore(false); }, [activeCategory, activeCountryFilter]);

  function handleLoadMore() {
    if (loadingMore) return;
    setLoadingMore(true);
    setTimeout(() => { setVisibleCount((v) => v + 6); setLoadingMore(false); }, 180);
  }

  function clearFilters() { setActiveCategory("All"); setActiveCountryFilter("all"); }
    ...
    <main className="bg-white text-[#1a1a2e]">
      ...
      <section className="relative overflow-hidden bg-[#f4f3ee] py-16 text-center sm:py-24">
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-4 text-center sm:px-6 lg:px-12">
          ...
          <p className="mt-5 text-xs uppercase tracking-[0.2em] text-[#6aa595] sm:mt-6">Insights Library</p>
          ...
          <p className="mx-auto mt-4 max-w-2xl text-sm text-[#5a6478] sm:text-base">
            Practical guidance for bookkeeping, tax, payroll, and finance operations across global markets.
          </p>
        </div>
      </section>
          ...
          <div className="flex flex-col gap-3 py-4 sm:flex-row sm:flex-wrap sm:items-center sm:gap-4 sm:py-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <CustomSelect
                ...
                onChange={(val) => { if (val === "detect") { alert("Location detection available soon."); return; } setActiveCountryFilter(val); }}
...
function CountryBadge({ country }) {
  if (!country || country === "All" || country === "General") {
    return <span className="bg-[#f8f9fa] border border-[#e2e4e9] text-xs rounded-full px-2 py-0.5 text-[#1a1a2e]">General</span>;
  }
  ...
  return (
    <span className="flex items-center gap-1 bg-[#f8f9fa] border border-[#e2e4e9] text-xs rounded-full px-2 py-0.5 text-[#1a1a2e]">
      {match?.flagSrc && <img src={match.flagSrc} alt="" className="rounded-sm" style={{ width: 14, height: 10 }} />}
      {country}
    </span>
  );
}
