...
export default function Footer() {
  ...
  return (
    <>
      <footer className="bg-[#0d0f14] py-14 text-[#8892a4] sm:py-20 lg:py-24">
          ...
          <div className="grid gap-10 sm:grid-cols-2 sm:gap-12 lg:grid-cols-4">
            ...
            <div className="sm:col-span-2 lg:col-span-1">
              <Link href="/" className="inline-flex items-center">
                ...
                <Image src="/logo-white.png" alt="ClariVex Solutions" width={180} height={52} className="h-9 w-auto object-contain sm:h-11" />
              </Link>
              ...
              <div className="mt-6">
                <p className="mb-3 text-xs uppercase tracking-widest text-[#5a6478]">Worldwide Presence</p>
                <div className="flex flex-wrap items-center gap-2">
                  {countries.map((c) => (
                    <button key={c.code} type="button" title={c.name} onClick={() => handleSelectCountry(c.code)}
                      className={`group relative rounded-md p-1 cursor-pointer transition-all duration-200 ${country === c.code ? "ring-2 ring-[#6aa595] bg-[#6aa595]/10" : "hover:ring-1 hover:ring-[#5a688e]/50 hover:bg-[#1e2330]"}`}>
                      <img src={c.flagSrc} alt={c.name} width={28} height={19} className="rounded-sm" style={{ width: 28, height: 19, display: "block" }} />
                      {country === c.code && <span className="absolute -bottom-1.5 left-1/2 h-1 w-1 -translate-x-1/2 rounded-full bg-[#6aa595]" />}
                    </button>
                  ))}
                </div>
                <button type="button" onClick={detectLocation} disabled={detecting}
                  ...
                  {detecting ? <Loader2 className="h-3 w-3 animate-spin text-[#6aa595]" /> : <LocateFixed className="h-3 w-3" />}
            ...
            <div>
              ...
              <div className="space-y-2.5">
                {serviceLinks.map((link) => (
                  <Link key={link.href} href={link.href} className="block text-sm transition-colors hover:text-white">{link.label}</Link>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2.5 text-sm">
                ...
                <a href="mailto:info@clarivex.net" className="block transition-colors hover:text-white">info@clarivex.net</a>
                <a href="tel:+919898028812" className="block transition-colors hover:text-white">+91 9898028812</a>
              </div>
            </div>
          </div>

          <div className="mt-12 border-t border-[#1e2330] pt-8 sm:mt-14">
            <div className="flex flex-col items-center gap-4 text-center text-xs text-[#8892a4] sm:flex-row sm:justify-between sm:text-left">
              <p className="flex items-center justify-center gap-1">
                <Link href="/admin" className="select-none cursor-default text-[#8892a4] hover:text-[#8892a4]" style={{ textDecoration: 'none' }}>&copy;</Link>
      ...
      <a href="https://wa.me/919104791017" target="_blank" rel="noopener noreferrer"
        className="fixed bottom-4 right-4 z-50 rounded-full bg-[#25D366] p-3 text-white shadow-2xl transition-transform hover:scale-110 sm:p-3.5"
