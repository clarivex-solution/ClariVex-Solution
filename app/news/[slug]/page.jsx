...
const getMoreNews = unstable_cache(
  async (slug, country, category) => {
    const more = await prisma.newsArticle.findMany({
      where: { NOT: { slug }, country },
      take: 3,
    ...
    if (more.length < 3) {
      const needed = 3 - more.length;
      const usedSlugs = [slug, ...more.map((n) => n.slug)];
      const fill = await prisma.newsArticle.findMany({
        where: { NOT: { slug: { in: usedSlugs } }, category },
        take: needed,
        orderBy: { publishedAt: "desc" },
        select: { slug: true, title: true, country: true, category: true },
      });
...
export async function generateMetadata({ params }) {
  ...
  try {
    ...
    console.warn("[news/slug] generateMetadata DB unavailable:", err.message);
  }
...
export default async function NewsArticlePage({ params }) {
  ...
  let dbPost = null;
  try {
    dbPost = await getNewsPost(slug);
  } catch (err) {
    console.warn("[news/slug] DB unavailable:", err.message);
  }
  ...
  let moreNews = [];
  try {
    moreNews = await getMoreNews(slug, post.country, post.category);
  } catch (err) {
    console.warn("[news/slug] more news DB unavailable:", err.message);
  }
          ...
          <article>
            <header className="mx-auto max-w-3xl">
              ...
              <div className="mt-6 flex flex-wrap items-center gap-6 border-b border-[#e2e4e9] pb-6">
                <div>
                  <p className="text-xs uppercase tracking-widest text-[#8892a4] mb-1">
                    Source
                  </p>
                ...
                <div>
                  ...
                  <time
                    ...
                    className="text-[#1a1a2e] font-semibold text-sm flex items-center gap-2"
                  >
                    {post.date}
                  </time>
                </div>
            ...
            <div className="mx-auto mt-8 max-w-3xl space-y-5 leading-relaxed text-[#5a6478]">
              ...
              <p className="mt-4 text-xs text-[#8892a4] italic">
                This article is sourced from {post.source}. Click below to read
                the complete article on their official website.
              </p>
            ...
            <div className="sticky top-28 space-y-6">
              ...
              <div className="bg-[#1a1a2e] rounded-2xl p-6 mt-4">
                ...
                <p className="mt-3 text-sm text-[#8892a4]">
                  Need help navigating these changes? Our team can assist with
                  compliance.
                </p>
                <Link
                  href="/#contact"
